...
{
    public class JHSNetworkWriter
    {
        const int k_MaxStringLength = 1024 * 32;
        JHSNetBuffer m_Buffer;
        static Encoding s_Encoding;
        static byte[] s_StringWriteBuffer;

        public JHSNetworkWriter()
        {
            m_Buffer = new JHSNetBuffer();
            if (s_Encoding == null)
            {
        ...
        public JHSNetworkWriter(byte[] buffer)
        {
            m_Buffer = new JHSNetBuffer(buffer);
            if (s_Encoding == null)
            {
                s_Encoding = new UTF8Encoding();
                s_StringWriteBuffer = new byte[k_MaxStringLength];
        ...
        public void Write(byte[] buffer, int count)
        {
            ...
            {
                if (NetConfig.logFilter >= JHSLogFilter.Error) { JHSDebug.LogError("JHSNetworkWriter :: Write: buffer is too large (" + count + ") bytes. The maximum buffer size is 64K bytes."); }
                return;
            }
        ...
        public void WriteBytesAndSize(byte[] buffer, int count)
        {
            ...
            if (count > UInt16.MaxValue)
            {
                if (NetConfig.logFilter >= JHSLogFilter.Error) { JHSDebug.LogError("JHSNetworkWriter :: WriteBytesAndSize: buffer is too large (" + count + ") bytes. The maximum buffer size is 64K bytes."); }
                return;
            }
        ...
        public void WriteBytesFull(byte[] buffer)
        {
            ...
            {
                if (NetConfig.logFilter >= JHSLogFilter.Error) { JHSDebug.LogError("JHSNetworkWriter :: WriteBytes: buffer is too large (" + buffer.Length + ") bytes. The maximum buffer size is 64K bytes."); }
                return;
            }
