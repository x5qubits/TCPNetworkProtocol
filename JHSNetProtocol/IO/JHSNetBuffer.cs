...
namespace JHSNetProtocol
{
    public class JHSNetBuffer
    {
        ...
        public JHSNetBuffer(byte[] buffer)
        {
            m_Buffer = buffer;
        }
        ...
        public byte ReadByte()
        {
            ...
            {
                throw new IndexOutOfRangeException("JHSNetworkReader:ReadByte out of range:" + ToString());
            }
        ...
        public void ReadBytes(byte[] buffer, uint count)
        {
            ...
            {
                throw new IndexOutOfRangeException("JHSNetworkReader:ReadBytes out of range: (" + count + ") " + ToString());
            }
        ...
        private void WriteCheckForSpace(ushort count)
        {
            ...
            while (Position + count >= newLen)
            {
                ...
                {
                    if (NetConfig.logFilter >= JHSLogFilter.Warning) JHSDebug.LogWarning("JHSNetworkBuffer :: Size is " + newLen + " bytes!");
                }
            }
        ...
        {
            return String.Format("JHSNetBuf sz:{0} pos:{1}", m_Buffer.Length, Position);
        }
    } // end NetBuffer
