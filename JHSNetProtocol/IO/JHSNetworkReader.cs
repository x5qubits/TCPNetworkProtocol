...
namespace JHSNetProtocol
{
    public class JHSNetworkReader
    {
        ...
        public JHSNetworkReader()
        {
            m_buf = new JHSNetBuffer();
            Initialize();
        }

        public JHSNetworkReader(JHSNetworkWriter writer)
        {
            m_buf = new JHSNetBuffer(writer.AsArray());
            Initialize();
        }

        public JHSNetworkReader(byte[] buffer)
        {
            m_buf = new JHSNetBuffer(buffer);
            Initialize();
        }
        ...
        public byte[] ReadBytes(int count)
        {
            if (count < 0)
            {
                throw new IndexOutOfRangeException("JHSNetworkReader ReadBytes " + count);
            }
